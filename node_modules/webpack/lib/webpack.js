...
const webpack = (options, callback) => {
	...
	if (Array.isArray(options)) {
		...
		// 初始化的准备工作 对webpack的options设置一些默认参数
		options = new WebpackOptionsDefaulter().process(options);

		compiler = new Compiler(options.context);
		compiler.options = options;
		//监听beforeRun  清理构建缓存 在entry-option和run 流程之间
		...
		// WebpackOptionsApply 初始化option 将配置的option参数转化为webpack内部的插件
		// 最终所有的插件都会变成complier对象上面的实例
		//  整个webpack编译的开始
